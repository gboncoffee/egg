...
func (m *ReduxPia) GetCurrentInstructionAddress() uint64 {
	return uint64(m.pc)
}

func (m *ReduxPia) ArchitectureInfo() machine.ArchitectureInfo {
...
func (m *ReduxPia) SetRegister(reg uint64, value uint64) error {
	if reg > 3 {
...
func (m *ReduxPia) GetRegister(reg uint64) (uint64, error) {
	if reg > 3 {
...
func (m *ReduxPia) SetMemory(addr uint64, value uint8) error {
...
func (m *ReduxPia) GetMemory(addr uint64) (uint8, error) {
	if addr > math.MaxUint8 {
...
func (m *ReduxPia) SetMemoryChunk(addr uint64, content []uint8) error {
...
func (m *ReduxPia) GetMemoryChunk(addr uint64, size uint64) ([]uint8, error) {
	end := addr + (size - 1)
	if end > math.MaxUint8 {
...
func (m *ReduxPia) LoadProgram(program []uint8) error {
	m.pc = 0
	return m.SetMemoryChunk(0, program)
}

func (m *ReduxPia) NextInstruction() (*machine.Call, error) {
	...
	case 0x5:
		m.SetRegister(0, uint64((uint8(r0v) & 0xf) | (imm << 4)))
	case 0x6:
	...
	case 0x7:
		m.SetRegister(0, uint64(r0v) + rav * rbv)
	case 0x8:
		if rbv == 0 {
...
func (m *ReduxPia) GetRegisterNumber(r string) (uint64, error) {
...
func assembleI(t assembler.ResolvedToken) (uint8, error) {
	...
	case "loop":
		op = 0x6
		imm = uint8(t.Address) - 1 - uint8(t.Args[0])
	default:
...
func assembleInstruction(code []uint8, addr int, t assembler.ResolvedToken) error {
	...
	case "addi", "ldui", "loop":
		bin, err = assembleI(t)
	case "brzr", "ld", "st", "not", "and", "or", "xor", "add", "sub", "slr", "srr", "mac":
		bin, err = assembleR(t)
	case "ecall":
		bin = 0x10
	default:
...
func (m *ReduxPia) Assemble(file string) ([]uint8, []assembler.DebuggerToken, error) {
